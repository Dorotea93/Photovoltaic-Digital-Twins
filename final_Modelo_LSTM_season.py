...
sheet = 'Hoja1'
sequence_length = 6  # Por ejemplo, 6 pasos de 15 minutos = 1,5h

# ----- Función para extraer estación -----
...
for estacion, grupo in df.groupby('Estacion'):
    if len(grupo) <= sequence_length + 8:
        print(f"Saltando {estacion}: muy pocos datos para secuencias LSTM ({len(grupo)})")
        continue
    ...
    # Normalizar solo dentro de la estación
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            val_loss = criterion(val_preds, y_val_torch)
    ...
    model.eval()
    with torch.no_grad():
        y_pred = model(X_val_torch).numpy()
        y_true = y_val_torch.numpy()
...
print("\nExcel actualizado con columnas 'Estacion' y 'Tipo' para LSTM estacional.")
