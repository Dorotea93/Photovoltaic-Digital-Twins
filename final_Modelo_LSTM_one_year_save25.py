# Funciona y registrado - Modelo LSTM
import pandas as pd
import numpy as np
...
# Función para crear secuencias temporales para LSTM
...
output_path = r'C:\Users\Usuario\Desktop\Trabajo_UNI\Gemelos digitales\Articulos\ART_3_redes neuronales\Datos_identificados_LSTM.xlsx'
...
# Paso 3: Definición del modelo LSTM
...
model = LSTMModel(input_size=3, hidden_size=50)
print(f"Modelo LSTM creado con {sum(p.numel() for p in model.parameters())} parámetros")
...
print("Mejor modelo LSTM cargado")
...
print('\n=== GUARDADO COMPLETADO ===')
print('Archivo guardado: best_model_lstm.pth')

# AÑADIR ESTO AL FINAL DEL SCRIPT
import joblib
joblib.dump(scaler, 'scaler_lstm.pkl')
print('Scaler guardado: scaler_lstm.pkl')

# Guardar predicciones y valores reales del 25 % de validación para LSTM
df_results = pd.DataFrame({
    'Actual': y_true.flatten(),
    'Predicted': y_pred.flatten(),
    'R2': r2_lstm   # Esta columna es opcional, para mostrar R² en el scatterplot
})
df_results.to_csv('lstm_results.csv', index=False)
print("Predicciones de validación guardadas en lstm_results.csv")
